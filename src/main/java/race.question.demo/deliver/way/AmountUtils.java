...
public class AmountUtils {
    ...
    public static void calculateLeftToOutcome(Income income, String zoneName, Result result) {
        if (!income.isZero()) {
            calculateToOutcome(income, income.getAmount(), zoneName, result);
        }
    }

    public static void calculateToOutcome(Income income, BigDecimal amount, String zoneName, Result result) {

        Outcome outcome;

        // 不够分，直接分余下的所有
        if (!enoughToDeliver(amount, income.getAmount())) {
            outcome = new Outcome(zoneName, income.getAmount());
            income.setAmount(BigDecimal.ZERO);
            result.addResult(outcome);
        } else {
            outcome = new Outcome(zoneName, amount);
